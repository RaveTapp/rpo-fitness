  ...
  if (confirm("Are you sure you want to remove this?")) {
    try {
      const response = await fetch(
        `http://localhost:3000/${
          isWorkout ? "deleteWorkout" : "deleteExercise"
        }`,
        {
          method: "POST",
          headers: {
...
async function editItem(old, newName, isWorkout) {
  try {
    const response = await fetch(
      `http://localhost:3000/${isWorkout ? "editWorkout" : "editExercise"}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    <>
      <div className={MenuCSS.horizontalFlex}>
        <Link
          ...
          to={
            props.isWorkout
              ? "/workout/" + props.n + "/exercise/1"
              : url.substring(0, url.length - 1) + props.n
          }
        >
          <h2 className={CSS.tekst}>{props.title}</h2>
        </Link>
        <svg
          className={rotateAmount == 0 ? MenuCSS.activeDots : MenuCSS.dots}
          fill="#EEEEEE"
          height="24px"
          width="24px"
      ...
      <div
        className={rotateAmount == 0 ? MenuCSS.active : MenuCSS.disabled}
        onClick={() => editPrompt(props)}
      >
      ...
      <div
        className={rotateAmount == 0 ? MenuCSS.active : MenuCSS.disabled}
        onClick={() => deleteItem(props.title, props.isWorkout)}
      >
